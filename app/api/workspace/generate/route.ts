...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!workspace.runway_state?.needs_generation) {
      return NextResponse.json({ already_generated: true });
    }

    console.log("GENERATE: starting content generation for", workspace.slug);
    ...
    return NextResponse.json({ success: true, runway_state, content_state, offer_card });
  } catch (err) {
